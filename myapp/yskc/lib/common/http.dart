...
class Http {
  ...
  Http() {
    ...
    this.dio.interceptors.add(InterceptorsWrapper(onRequest: (RequestOptions options) {
          print('-----------------------------${options.path}');
          ...
          switch (err.type) {
            case DioErrorType.RESPONSE:
              if (err.response != null && err.response.statusCode == 401) {
                ...
                Global.navigatorKey.currentState.pushNamedAndRemoveUntil('error', (route) => route == null);
              }
              break;
